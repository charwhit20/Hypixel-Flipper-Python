...
#processor for decode_inventory_data
def unpackNBT(tag):
...
#function that decodes 'item_bytes' tag in hypixel JSON call
def decodeItemBytes(raw):
    data = NBTFile(fileobj = io.BytesIO(base64.b64decode(raw)))
    unpackedNBT = unpackNBT(data.tags[0])
    itemDict = dict(unpackedNBT[0])
    return itemDict

#end of item decoding
...
#compact calling of hypixel getJson
class Auction:
    def __init__(self):
        pass   
    def getLiveAuctions(self, PageNumber):
        return hypixel.getJSON("skyblock/auctions", page = str(PageNumber))
    def getPastAuctions(self):
        return hypixel.getJSON("skyblock/auctions_ended")
    def getBazaar(self):
        return hypixel.getJSON("skyblock/bazaar")
...
#add support for shit reforges and enchants
#cull all data after 604,800,000 timestamp (milliseconds)

#keeps script running
while True:
    print("\nGathering data...")
    #add all items to a dictionary, with a class containing the data as the key value, so that you can access items by key, and there arent tons of objects stored, just one object per item updated each time
    Items = {}
    Enchants = {}
    AttShards = {}
    #for pets, find the graph that roughly translates to the price of them at any level, try to assign ah price for live ones based off it
    Pets = {}
    Runes = {}
    pastAID = 0
    #keep current time in variable to reduce calls
    current = time.time()
    try:
        openfile = open(filename, 'rb')
        temp = pickle.load(openfile)
        for i in temp:
            #initialize id in Items
            Items[i] = {'average': 0, 'total': 0, 'items': []}
            total = 0
            Items[i]['items'] = []
            #for each stored auction [price, timestamp]
            for j in temp[i]['items']:
                #if timestamp less than a week old
                if j[1] - (current * 1000) < 604800000:
                    #append to stored auctions, update values
                    Items[i]['total'] += 1
                    Items[i]['items'].append([j[0], j[1]])
                    total += j[0]
            #update average
            if Items[i]['total'] > 0:
                Items[i]['average'] = total / Items[i]['total']
        openfile.close()
        print("File load successful")
    except:
        print("File did not exist, continuing")

    #script gathers 20 times before adding to file
    for i in range(20):
        #gathers auctions that ended in past 60 seconds
        pastAuctions = AUCTIONS.getPastAuctions()
        #for each auction in those that ended in past 60 seconds
        for x in pastAuctions['auctions']:
            #only count bin sells and clean items
            #must add support for shit reforges and enchantments to get more data
            ExtraAttributes = decodeItemBytes(x['item_bytes'])['tag']['ExtraAttributes']
            ID = ExtraAttributes['id']
            print(ID)
            if x['bin'] == False or 'enchantments' in ExtraAttributes or 'modifier' in ExtraAttributes:
                continue
            if x['auction_id'] == pastAID:
                break
            if ID in Items:
                #update total and price
                #inc total
                Items[ID]['total'] += 1;
                #add new item to item list, this is in order to keep track of timestamp so you can cull old data
                Items[ID]['items'].append([x['price'], x['timestamp']])
                total = 0
                #collect average of all items in item list, slower collection but easier access for flipper
                for z in Items[ID]['items']:
                    total += z[0]
                Items[ID]['average'] = total / Items[ID]['total']

            else:
                #append to dict the id as a key pointing to a dict with structure ID: {average, total, items[]}, items is a multidim array containing [price, timestamp]
                Items[ID] = {'average': x['price'], 'total': 1, 'items': [[x['price'], x['timestamp']]]}
        #stores auction id of first auction in list, this will be the ending point for the next iteration
        pastAID = AUCTIONS.getPastAuctions()['auctions'][0]['auction_id']
        print("Iteration " + str(i + 1) + " of 20 complete")
        time.sleep(30)
    print("Data gathered, attempting to dump to pickle")
    try:
        f = open(filename, 'wb')
        pickle.dump(Items, f)
        f.close()
